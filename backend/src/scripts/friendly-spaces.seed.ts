...
import {
  FriendlySpaceModel,
  type FriendlySpaceDocument,
} from '../modules/friendly-spaces/friendly-space.model.js';

type FriendlySpaceSeed = Omit<
  FriendlySpaceDocument,
  'createdAt' | 'updatedAt'
>;

const friendlySpaces: FriendlySpaceSeed[] = [
{
    ...
    },
    {
      ...
      categoryKey: 'cafe-restaurante-comida-para-llevar',
    ...
    {
      name: 'Ángara Camós',
      categoryKey: 'ropa-zapatos-y-complementos',
    ...
    {
      name: 'Xipinines',
      categoryKey: 'librerias-papelerias-jugueteria-regalos',
    ...
    },
    {
      ...
      categoryKey: 'centros-de-estetica-peluquerias-perfumerias',
    ...
    },
    {
      ...
      categoryKey: 'clinicas-y-espacios-de-salud',
    ...
    {
      name: 'Caixa Popular',
      categoryKey: 'servicios-financieros-consultorias',
...
async function seedFriendlySpaces(): Promise<void> {
  ...
  try {
    ...
    await FriendlySpaceModel.deleteMany({});

    const insertedFriendlySpaces = await FriendlySpaceModel.insertMany(
      friendlySpaces,
      {
        ordered: true,
      },
    ...
    console.log(
      `Seed completado: ${insertedFriendlySpaces.length} espacios amigos insertados.`,
    );
  } catch (error) {
    console.error('Error al ejecutar el seed de espacios amigos:', error);
    process.exitCode = 1;
  } finally {
